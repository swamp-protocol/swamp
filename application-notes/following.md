...
v0.4.0 ships `Following:` with a structured-only body and no per-entry metadata. If practice surfaces a real need for per-entry annotations (tags, whys, display names), a later release can extend the body grammar additively — for example, with an optional third column whose semantics are reserved.

The deliberate stance for v0.4.0 is *don't pre-impose structure*. Authors with rich opinions about specific entries publish commentary posts that reference the `Following:` post. The shape of the artifact follows the shape of the thought.
