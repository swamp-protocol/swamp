...
```
Content-Type: application/swamp; kind=profile; v=0.3.0
...
- **XID-based scoping.** When Swamp adopts XID (a later-release target; see `related-work/hubert.md`), XID's native support for multiple key contexts may change the shape of this decision. A single XID can hold several signing keys with different scopes, which maps more naturally onto principal-and-agents than `did:key` does. This note's recommendations are written for the `did:key` baseline; the XID story will get its own note when the tooling lands. v0.4.0 readied the `DID:` header for multi-key DIDs by permitting an optional fragment (`did:key:z6Mk...#z6Mk...` or, in a later release, `did:web:foo.example.com#key-1`) — see SPEC §3 — but the principal-and-agent shape decision is still about how many DIDs to mint, not about fragment syntax.
...
*Application note accompanying the Swamp v0.3.0 specification.*
