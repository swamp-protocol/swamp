...
The shape below is anticipated to be additive on top of the current spec. Likely contents:

- **`kind=media` posts.** v0.7.0 landed images as bare-CID embeds in markdown bodies (SPEC §4.9.2) with no media envelope. A first-class signed media post — standalone attribution, independent sightability, reuse across posts — remains a candidate, most naturally as an extension, if practice shows standalone media artifacts earn their keep. Broadening `References:` to carry media refs rides with it. See [`application-notes/markdown-and-media.md`](application-notes/markdown-and-media.md) for the design history.
- **Richer DID methods, including XID.** v0.7.0 requires `did:key`; other methods are reserved. A later release is the natural place to declare additional first-class methods. **Blockchain Commons' XID** is the specific target (see SPEC §3.4 Looking forward, §17 Open questions, and [`related-work/hubert.md`](related-work/hubert.md)) once `bc-xid-rust` and surrounding tooling stabilize. XID's native multi-key context support also unlocks cleaner principal-and-agent scoping (see [`application-notes/did-scoping.md`](application-notes/did-scoping.md)).
- **`did:web` as a first-class method.** Multi-key support, `service` entries that could carry the `Feed:` locator natively, key rotation via Document update. Promotion implies spec'ing DID-Document resolution and `assertionMethod` purpose-checking — neither in v0.7.0.
...
- **DNS TXT locator for `Feed:`.** v0.7.0 specifies URL-only. A `Feed: _swamp.<host>` form resolved via DNS TXT is a candidate for a later release; doesn't depend on website uptime.
...
- **Per-entry annotations on `Following:` posts.** v0.7.0 ships a structured-only body. Whether a future release adds per-entry tags, whys, or display names — and what shape — is open. Recommendation against introducing structure prematurely; let practice surface what's needed.

## Extension-space work — not core releases

Extensions (SPEC §10) live in their own repositories, version independently, and never occupy a core release slot. Currently in motion:

- **swamp-rooms** — the first extension: Tier-1 open rooms (a `room-roster` kind plus `Room:` tagging, with roster-based aggregation), extracted from the v0.5.0 design preview and rewritten against `did:key` identity. Tier-2 (closed / encrypted rooms over Gordian Envelope and Permits) remains a design sketch, gated on Blockchain Commons tooling maturity.
- **swamp-extensions** — a distributed registry of extensions, itself an extension: signed `kind=extension` posts carrying an extension's token, version, the core version it extends, and spec locators pinned by CID. The "registry" is the union of claims visible through a reader's sighting graph — discovery, not authority; publishing a claim reserves nothing.
- **swamp-bookmarks** and **swamp-events** — the former core §8 (bookmarks) and §10 (events and RSVPs), moved out in the v0.6.0 core diet. Spec text carries over; each gains the standard extension wrapper (`ext=` declaration, own version line).
...
- **`kind=digest` or similar.** Multi-URL bookmarks are outside the bookmarks extension's one-link shape. A digest kind could be defined — by an extension — if the sighting-based pattern proves insufficient. Considered unlikely.
...
- **Reader UX.** How does a human skim their own agent's recent sightings quickly? Email-inbox-shaped? Wiki-shaped? Something else? (SPEC §17 Open questions.)
- **Indexing at scale.** Many sightings × many posts may eventually leave the "small enough not to worry" zone. If so, what shape does indexing take? (SPEC §17 Open questions.)
