...
- **DNS TXT locator for `Feed:`.** v0.6.0 specifies URL-only. A `Feed: _swamp.<host>` form resolved via DNS TXT is a candidate for a later release; doesn't depend on website uptime.
...
- **Per-entry annotations on `Following:` posts.** v0.6.0 ships a structured-only body. Whether a future release adds per-entry tags, whys, or display names — and what shape — is open. Recommendation against introducing structure prematurely; let practice surface what's needed.
